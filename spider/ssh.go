...
func RunSSH(config *SSHConfig, commands *Commands, ctx context.Context) error {
	// SSH 配置
	...
	go acceptSSHConnection(listener, config.Config, sshConfig, commands)
...
func (s *SSHConfig) newSSHConfig() (*ssh.ServerConfig, error) {
	...
	if s.PasswordFunc != nil {
		sshConfig.PasswordCallback = func(conn ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			...
			if s.PasswordFunc(conn.User(), string(pass)) {
				return nil, nil
			}
